<template>
  <div class="bgView">
    ...
    <div class="bgScored">
      <div>
        <img  class="headerImg" src="/sell/image/fendou.jpg">
        <div class="grade-box">
    ...
    <div class="line"></div>
    <div class="inputText">
      <textarea v-bind:maxlength="Surplus" @input="descArea" v-model="inputText" name="abstract" id="abstract" placeholder="亲,来评价下「奋斗小馆」的用餐体验吧,可以从菜品口味和服务等方面描述哦~"></textarea>
      <!-- <p>还可以评价{{Surplus}}字</p> -->
      <div class="selectPic">
        <up-Load :multiple="true" :max=3 :list="imgList" ref="upload"></up-Load>
      </div>
    </div>
    <!-- <div class="bgName">
    ...
    <div class="severce">
      ...
      <div class="severScoreInfo">{{severScoreInfo}}</div>
    </div>
    <div class="publish" @click="publicComment">评论</div>
  </div>
</template>
...
export default {
  data: function () {
    return {
      ...
      Surplus: 200,
      profiletitle: '商品评价',
      scoreStartNum: 0, // 商品评分
      severStartNum: 0, // 服务评分
      imgList: [],      //图片地址
      newSet: '     ',
      isBack: true,
      isSet: true,
  ...
  methods: {
    ...
    rating: function (index, string) {
      if (string === 'starts') {
        ...
        if (this.severStartNumv === 1) {
          this.severScoreInfo = '很差'
        } else if (this.severStartNum === 2) {
          this.severScoreInfo = '差'
        } else if (this.severStartNum === 3) {
          this.severScoreInfo = '一般'
        } else if (this.severStartNum === 4) {
          this.severScoreInfo = '满意'
        } else if (this.severStartNum === 5) {
          this.severScoreInfo = '很满意'
        }
      }
    },
    descArea () {
    ...
    publicComment () {
      ...
      // this.inputText  代表心得
      if(this.inputText == ''){
        alert("亲~评论不能为空哦，说点东西吧！！")
        return
      }
      if(this.scoreStartNum == 0){
        alert("亲~商品评分不能为空哦，给个好评呗！！")
        return
      }
      if(this.severStartNum == 0){
        alert("亲~服务评分不能为空哦，给个好评呗！！")
        return
      }
      this.$http.post(this.HOST+'/comment/add',{
           orderId: this.$route.params.orderId,
           openid: getCookie('openid'),
           context: this.inputText,
           scoreStartNum: this.scoreStartNum,
           severStartNum: this.severStartNum,
           commentImag1: this.$refs.upload.imgUrls[0],  //图片地址
